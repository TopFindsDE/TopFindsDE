...
(() => {
  ...
  /** "Neu entdeckt": die Produkte vom neuesten added-Datum, max. 10.
      Hat noch kein Produkt ein Datum, werden die letzten 10 gezeigt. */
  const newestProducts = () => {
    const dated = PRODUCTS.filter((p) => p.added);
    if (!dated.length) return PRODUCTS.slice(-10).reverse();
    const latest = dated.map((p) => p.added).sort().pop();
    return dated.filter((p) => p.added === latest).slice(0, 10);
  };
...
if (items.length === 0) {
  ...
  const buildFilters = () => {
    ...
    const chips = [{ slug: "alle", name: "Alle" }, ...CATEGORIES];
